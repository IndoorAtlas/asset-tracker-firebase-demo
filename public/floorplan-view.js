...
function flooPlanAutoZoomLevel(floorPlan) {
  const height = floorPlan.image.heightMeters;
  const width = floorPlan.image.widthMeters;
  const diameter = Math.sqrt(height*height + width*width);
  const tileWidth19 = Math.cos(floorPlan.image.center.lat*Math.PI/180.0) * 76.0;
  const tilesPerScreen = 5; // about

  const nScreens = diameter / (tilesPerScreen * tileWidth19);
  const zoom = 19 + Math.round(-Math.log2(nScreens));

  return Math.max(0, Math.min(zoom, 21));
}
...
function FloorPlanView(map) {
  ...
  this.show = function(floorPlan) {
    ...
    map.setView(L.latLng(
      ...
      flooPlanAutoZoomLevel(floorPlan));
  }
